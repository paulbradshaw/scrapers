...
#Parslepy rules for scraping Gorkana. These also set the fields.
rules = {
    "job_links(ul.jobsList li)":[
        {
        "job_id":"@id",
        "job_title": "h4",
        "job_link": "h4 a @href",
        "location": "ul.detail.hasImage li",
        "remuneration": "ul.detail.hasImage li.last",
        "description":"p",
        "employer":"ul.meta li"
        }
    ]
...
#A list of keywords to loop through and add to a URL to scrape
keywords = ['Work+experience', 'volunteer', 'voluntary', 'Voluntary+worker', 'unpaid', 'expenses']
for keyword in keywords:
#scraping URL with salary less than 10k and keyword inserted from loop
    html = requests.get('http://www.gorkanajobs.co.uk/searchjobs/?Keywords='+keyword+'&SalaryBand_Min=-1&SalaryBand_Max=10000')
